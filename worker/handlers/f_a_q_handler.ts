...
export class FAQHandler extends BaseMenuCodeHandler<MenuCode.FAQ> {
    constructor(menuCode : MenuCode.FAQ) {
        super(menuCode);
    }
    async handleCallback(params : CallbackHandlerParams, context: FetchEvent, env: Env) : Promise<BaseMenu|ReplyQuestion|void> {
        return new Menus.MenuFAQ({ userID : params.getTelegramUserID(), chatID : params.chatID }, env);
    }
}
